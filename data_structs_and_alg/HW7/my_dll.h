...
int dll_delete(dll_t* dll, void* value)
{
	...
	while (current != NULL) {
		...
		if (current->data == value) {
			// Found a node with the specified value, so delete it
			if (current == dll->head) {
				...
				if (next_node != NULL) {
					...
					// If there's no next node, update the tail
					dll->tail = NULL;
				}
			}
			else if (current == dll->tail) {
				// If it's the tail node, update the tail pointer
				dll->tail = current->previous;
				current->previous->next = NULL;
			}
			else {
				// If it's an intermediate node, adjust the pointers of previous and next nodes
