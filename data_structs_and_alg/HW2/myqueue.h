...
queue_t* create_queue(unsigned int _capacity)
{
    ...
    myQueue->back = _capacity - 1;  // Initialize rear as the last index
...
void free_queue(queue_t* q) {
    if (q) {
        if (q->data) {
            free(q->data);  // Free the memory used by the array
        }
        free(q);  // Free the memory used by the queue structure
    }
}
